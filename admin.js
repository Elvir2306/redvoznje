...
function deleteRoute() {
    if (!currentFrom || (!currentTo && !toNewInput.value.trim())) {
        alert('Odaberi rutu za brisanje!');
        return;
    }
...
function renderAllRoutes() {
    ...
    if (Object.keys(busData).length === 0) {
        routesList.innerHTML = '<p>Nema dodanih ruta još!</p>';
        return;
    }
    ...
    for (const from in busData) {
        for (const to in busData[from]) {
            ...
            title.innerHTML = `${from} → ${to} <span class="route-edit" data-from="${from}" data-to="${to}">Uredi</span>`;
