...
function updateClock() {
    ...
    return `${String(now.getHours()).padStart(2, '0')}:${minutes}:${seconds}`;
}
...
function showResults() {
    ...
    if (!from) {
        resultsDiv.innerHTML = `
            <div class="welcome-message">
                ...
                <p>Odaberi željenu destinaciju</p>
            </div>`;
        return;
    }
    ...
    if (to) {
        ...
        if (busData[from] && busData[from][to]) {
            ...
            html += `</div><br/><i>(*)</i> ne saobrača praznikom i vikendom</div>`;
        } else {
            html = `
            <div class="welcome-message">
                <i class="fas fa-exclamation-circle"></i>
                <p>Nema pronađenih linija ${from} -> ${to}</p>
            </div>`;
        }
    } else {
        // Show all routes
        if (busData[from]) {
            ...
            <div class="welcome-message">
                <i class="fas fa-exclamation-circle"></i>
                <p>Nema pronađenih linija ${from}</p>
            </div>`;
        }
    }
